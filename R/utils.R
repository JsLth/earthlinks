...
opts_cache <- new.env(parent = emptyenv())
options_cached <- function(...) {
  old <- options(...)
  
  for (opt in names(old)) {
    assign(opt, old[[opt]], envir = opts_cache)
  }
}


options_restore <- function() {
  old <- as.list(opts_cache)
  options(old)
}
